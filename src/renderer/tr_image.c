...
static void LoadJPG( const char *filename, unsigned char **pic, int *width, int *height ) {
	...
	*pic = NULL;
	...
	if ( setjmp( jerr.setjmp_buffer ) ) {
		// R_JPGErrorExit already printed the message; the image buffer is pooled
	...
	cinfo.out_color_space = JCS_EXT_RGBA;   // libjpeg-turbo fills the alpha for us
#else
	cinfo.out_color_space = JCS_RGB;
#endif
	jpeg_start_decompress( &cinfo );

	if ( cinfo.output_width < 1 || cinfo.output_height < 1 ) {
		ri.Error( ERR_DROP, "LoadJPG: %s has a bad size (%u x %u)", filename, cinfo.output_width, cinfo.output_height );
	}

	rowStride = cinfo.output_width * 4;
	out = R_GetImageBuffer( rowStride * cinfo.output_height, BUFFER_IMAGE );

	while ( cinfo.output_scanline < cinfo.output_height ) {
		byte *dst = out + rowStride * cinfo.output_scanline;
		JSAMPROW rows[1];
		...
		if ( cinfo.output_components == 4 ) {
			...
			// expand gray / RGB rows to RGBA
			int i, c = cinfo.output_components;
			byte *row = R_GetImageBuffer( cinfo.output_width * c, BUFFER_RESAMPLED );

			rows[0] = row;
			jpeg_read_scanlines( &cinfo, rows, 1 );
			for ( i = 0; i < (int)cinfo.output_width; i++ ) {
				dst[i * 4 + 0] = row[i * c];
				dst[i * 4 + 1] = row[i * c + ( c > 1 ? 1 : 0 )];
				dst[i * 4 + 2] = row[i * c + ( c > 1 ? 2 : 0 )];
				dst[i * 4 + 3] = 255;
			}
		}
	}
...
Encodes a bottom-up RGBA image into a malloc'ed JPEG stream.
The setjmp lives here so that the buffer pointer is not a local of the
function that longjmps.
=================
*/
static qboolean R_CompressJPG( int quality, int image_width, int image_height, const byte *image_buffer, unsigned char **outbuf, unsigned long *outsize ) {
	...
	while ( cinfo.next_scanline < cinfo.image_height ) {
		// the frame buffer read back is bottom up
		const byte *src = image_buffer + ( cinfo.image_height - 1 - cinfo.next_scanline ) * row_stride;
#ifdef JCS_EXTENSIONS
		row_pointer[0] = (JSAMPROW)src;
#else
		{
			byte *row = R_GetImageBuffer( image_width * 3, BUFFER_RESAMPLED );
			int i;
...
void SaveJPG( char * filename, int quality, int image_width, int image_height, unsigned char *image_buffer ) {
	...
	if ( R_CompressJPG( quality, image_width, image_height, image_buffer, &outbuf, &outsize ) ) {
		ri.FS_WriteFile( filename, outbuf, outsize );
	} else {
		ri.Printf( PRINT_WARNING, "SaveJPG: failed to encode %s\n", filename );
	}
	free( outbuf );
}
